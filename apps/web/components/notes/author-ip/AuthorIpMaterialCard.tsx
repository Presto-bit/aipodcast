...
import { resumeCardSubtitle, tryParseResume } from "./resumeTypes";
...
export default function AuthorIpMaterialCard({ material, readOnly, busy, onPreview, onDelete }: Props) {
  ...
  const subtitle = resume ? resumeCardSubtitle(resume) : null;
  ...
  return (
    <li className="relative rounded-xl border border-line/80 bg-surface shadow-sm transition hover:border-brand/25">
      <div className={cn("absolute bottom-0 left-0 top-0 w-1 rounded-l-xl", barClass)} aria-hidden />
      <button
        type="button"
        className="block w-full px-3 py-2.5 pl-4 text-left"
      ...
      {!readOnly ? (
        <div className="absolute right-1 top-1" data-material-card-menu>
          <button
            type="button"
