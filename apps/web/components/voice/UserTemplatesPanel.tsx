...
import {
  ...
  type CreativeBundle,
  isUserPersonaStyleTemplateCategory,
...
/** 人设风格（原「风格模板 / 加入创意」）管理：与音色管理页「人设风格」标签共用 */
export default function UserTemplatesPanel() {
  ...
  const refresh = useCallback(() => {
    setItems(listUserTemplates().filter((t) => isUserPersonaStyleTemplateCategory(t.category)));
  }, []);
  ...
  function onAdd(e: React.FormEvent) {
    ...
    const autoLabel = `人设风格 · ${savedAt.toLocaleString("zh-CN", { hour12: false })}`;
    addUserTemplate({
      id,
      label: autoLabel,
      description: "仅人设风格参数",
  ...
  return (
    <div className="mx-auto min-h-0 w-full max-w-3xl">
      <h1 className="text-2xl font-semibold text-ink">人设风格</h1>

      <form className="mt-6 space-y-4" onSubmit={onAdd}>
        <div className="rounded-2xl border border-line bg-surface p-4">
          <h2 className="text-sm font-semibold text-ink">人设风格</h2>
          <p className="mt-1 text-[11px] text-muted">
            至少填写一项后保存为方案；在 AI 播客 / 笔记本「人设风格」下拉里可选中该自定义方案。
          </p>
      ...
      <section className="mt-8">
        <CollapsibleCreativeSection
          ...
          title="自定义人设风格"
          subtitle={<>仅本人可见；已保存 {items.length} 条。登录后随用户偏好同步到服务器。</>}
        >
          {items.length === 0 ? (
            <p className="text-sm text-muted">暂无自定义方案，请在上方填写并保存。</p>
          ) : (
            items.map((t) => {
