/**
 * 营销首屏装饰：无远程大图。叙事为「多源资料 → 引用式回答 → 波形 / 播客 → 多形态输出」。
 */
  ...
  return (
    ...
    >
      <div className="flex shrink-0 items-center gap-1.5 border-b border-line/80 bg-fill/50 px-3 py-2.5">
        ...
        <span className="ml-1.5 min-w-0 truncate text-[11px] font-semibold tracking-tight text-ink/90">
          PrestoAI · 知识库 → 创作
        </span>
      </div>

      <div className="relative grid min-h-0 flex-1 grid-cols-[minmax(0,34%)_1fr] gap-0">
        <div
          className="absolute bottom-3 left-[34%] top-12 z-0 w-px bg-gradient-to-b from-transparent via-brand/25 to-brand/10"
          aria-hidden
        />
        <div className="relative z-[1] min-h-0 border-r border-line/70 bg-fill/35 p-2.5 sm:p-3">
          ...
          <ul className="mt-2 space-y-2">
            {[
              ...
              <li
                key={row.t}
                className="flex items-center justify-between gap-1 rounded-md border border-line/60 bg-surface/80 px-1.5 py-1 text-[10px] text-ink/90"
              >
                <span className="truncate font-medium">{row.t}</span>
                <span className="shrink-0 rounded bg-brand/12 px-1 py-px text-[9px] font-semibold text-brand">{row.s}</span>
              </li>
            ))}
          </ul>
          <p className="mt-2 text-[9px] leading-snug text-muted">仅处理你上传的资料</p>
        </div>
        ...
        <div className="relative z-[1] flex min-h-0 flex-col bg-gradient-to-br from-brand/[0.07] via-transparent to-transparent p-2.5 sm:p-3">
          <div className="rounded-lg border border-line/70 bg-surface/90 p-2 shadow-sm ring-1 ring-black/[0.03] dark:ring-white/[0.04]">
            <p className="text-[9px] font-semibold text-muted">回答（示意）</p>
          ...
          <div className="mt-auto pt-3">
            ...
            <p className="mt-1.5 text-center text-[9px] text-muted">多角色配音 · 成片进「我的作品」</p>
          </div>
        </div>
      </div>
    </div>
