...
function parseTrashSection(raw: string | null): TrashSection {
  return String(raw || "").trim().toLowerCase() === "notes" ? "notes" : "works";
}
...
export default function WorksTrashPageClient() {
  const { t } = useI18n();
  ...
  const [section, setSection] = useState<TrashSection>(() => parseTrashSection(searchParams?.get("section") ?? null));
  ...
  useEffect(() => {
    setSection(parseTrashSection(searchParams?.get("section") ?? null));
  }, [searchParams]);

  const setTrashSection = useCallback(
    (next: TrashSection) => {
      setSection(next);
      router.replace(next === "notes" ? "/works/trash?section=notes" : "/works/trash", { scroll: false });
    },
    [router]
  );
  ...
  async function bulkRestoreWorks(ids: string[]) {
    ...
    try {
      ...
      await load();
      if (failed > 0) setErr(`作品恢复完成，但有 ${failed} / ${ids.length} 条失败，可重试。`);
    } catch (e) {
      setErr(String(e instanceof Error ? e.message : e));
    } finally {
      setBusy(null);
  ...
  async function bulkPurgeWorks(ids: string[]) {
    ...
    try {
      ...
      const failed = results.filter((r) => !r.ok).length;
      await load();
      if (failed > 0) setErr(`作品删除未完成：${failed} / ${ids.length} 条失败，请重试。`);
    } catch (e) {
      setErr(String(e instanceof Error ? e.message : e));
    } finally {
      setBusy(null);
  ...
  const empty = !loading && notes.length === 0 && works.length === 0;

  return (
    <main className="mx-auto min-h-0 w-full max-w-4xl px-3 pb-10 sm:px-4">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <Link href="/works" className="text-sm text-brand hover:text-brand/80">
          {t("trash.backToWorks")}
        </Link>
      </div>
      <h1 className="text-2xl font-semibold text-ink">{t("nav.trash")}</h1>
      <p className="mt-2 text-sm text-muted">{t("trash.description")}</p>

      <div className="mt-4 flex flex-wrap gap-1.5">
        <button
          type="button"
          className={chipClass(showWorks, "sm")}
          onClick={() => setTrashSection("works")}
        >
          {t("trash.tabWorks")}
        ...
        <button
          type="button"
          className={chipClass(showNotes, "sm")}
          onClick={() => setTrashSection("notes")}
        >
          {t("trash.tabNotes")}
      ...
      {loading ? (
        ...
        <EmptyState className="mt-8" title={t("trash.emptyTitle")} description={t("trash.emptyDescription")} />
      ) : (
        <div className="mt-6 space-y-6">
          {showWorks ? (
            <section>
              <h2 className="mb-2 text-sm font-semibold text-ink">{t("trash.sectionWorks")}</h2>
              {works.length === 0 ? (
                <p className="text-xs text-muted">{t("trash.noWorks")}</p>
              ) : (
                <>
          ...
          {showNotes ? (
            <section>
              <h2 className="mb-2 text-sm font-semibold text-ink">{t("trash.sectionNotes")}</h2>
              <p className="mb-2 text-xs text-muted">{t("trash.notesRestoreHint")}</p>
              {notes.length === 0 ? (
                <p className="text-xs text-muted">{t("trash.noNotes")}</p>
              ) : (
                <>
                  <div className="mb-2 flex flex-wrap items-center gap-2 rounded-lg border border-line/80 bg-fill/40 px-2 py-2 text-xs text-ink">
                    <label className="flex cursor-pointer items-center gap-2">
