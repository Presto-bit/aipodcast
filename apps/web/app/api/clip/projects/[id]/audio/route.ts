...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  try {
    const upstream = await fetchOrchestrator(`/api/v1/clip/projects/${encodeURIComponent(id)}/audio`, {
      method: "POST",
      body: buf,
      payload: "{}",
      headers: {
        "x-clip-filename": encodeClipFilenameForHttpHeader(decoded, "upload.mp3"),
