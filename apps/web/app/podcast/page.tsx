...
import CreativeTemplatePicker from "../../components/studio/CreativeTemplatePicker";
...
const MAIN_TEXT_PLACEHOLDER =
  "第 1 步：输入主题或粘贴素材。建议直接写清受众、目标和关键观点。";
...
export default function PodcastPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (typeof parsed.defaultScriptTargetChars === "number" && Number.isFinite(parsed.defaultScriptTargetChars)) {
        const n = Math.max(200, Math.min(9999, Math.round(parsed.defaultScriptTargetChars)));
  ...
  function toggleNote(id: string) {
    setSelectedNoteIds((prev) => {
      ...
      if (prev.length >= noteRefCap) {
        setTaskPhase(`已达套餐资料上限：最多 ${noteRefCap} 本笔记（基础 1 / Pro 5 / Max 10）`);
        return prev;
      }
      return [...prev, id];
    });
  ...
  async function uploadNoteFile(file: File | null) {
    ...
    if (!targetNb) {
      applyTaskFromEvent("请先在「笔记播客」页新建笔记本，再上传资料文件");
      return;
    }
    setUploadBusy(true);
    setUploadProgress(0);
    ...
    <main className="mx-auto min-h-0 w-full max-w-6xl px-3 pb-10 sm:px-4">
      <div className="mb-6 text-center">
        ...
        <p className="mt-2 text-sm text-muted">
          3 步完成：输入内容 → 选择风格 → 开始生成
        </p>
      </div>

      <section className="relative overflow-visible rounded-2xl border border-line bg-surface shadow-card-sm">
        <div className="flex min-h-0 flex-col p-4 md:p-5">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
            ...
            <span className="rounded-full bg-fill px-2 py-1 text-muted">第 2 步 选择风格</span>
          ...
          <div className="relative mt-4 border-t border-line pt-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                    ...
                    {renderFloatingPanel(
                      ...
                      <>
                        ...
                        {voiceOptions.length === 0 ? (
                          <p className="text-xs text-amber-700">正在加载音色列表…若长时间为空，请检查 /api/default-voices 与网络。</p>
                        ) : null}
                        {speakerMode === "single" ? (
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="rounded-lg border border-line bg-fill/70 p-2.5">
                          ...
                          <p className="mb-1 text-xs text-muted">
                            资料 {selectedNoteIds.length}/{noteRefCap}（Free 1 · Pro 5 · Max 10）
                          </p>
              ...
              <button
                ...
                className="inline-flex min-w-[6.25rem] shrink-0 items-center justify-center gap-2 self-end rounded-full bg-cta px-3 py-2 text-xs font-medium text-white shadow-card-sm transition hover:scale-105 hover:bg-cta/90 sm:ml-1 sm:self-start"
      ...
      <section className="mt-6">
        ...
        {showTaskPanel ? (
          <div className="mb-4 rounded-2xl border border-brand/25 bg-fill/90 p-4 shadow-card-sm">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-brand">生成中状态</h3>
            <p className="mt-2 text-sm text-ink">{taskPhase || (busy ? "正在生成，请稍候" : "—")}</p>
            {busy || taskProgressPct > 0 ? (
              <p className="mt-1 text-[11px] leading-relaxed text-muted">
                当前阶段：脚本生成 → 语音合成 → 混音封面；长稿或网络素材会更久。
              </p>
            ) : null}
            <div className="mt-3">
