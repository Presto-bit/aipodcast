...
describe("studioAgentStructured", () => {
  it("parse, resolve, and fallback contracts", () => {
    ...
    assert(kept.kind === "silent", "V2: draft ask_user suppresses for open-ended generate");

    const fallback = draftAskFallbackText(vagueWork, "帮我想想", '{"kind":"silent"}');
    assert(fallback === null, "V2: vague draft silent has no fallback when auto-generate");
