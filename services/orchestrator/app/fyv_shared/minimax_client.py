...
6. 【话轮与叙事】Speaker1 与 Speaker2 须功能互补（例如一方偏提问、过渡与微型小结，另一方偏拆解、举例与界定），避免轮流复述同一信息层；每隔若干轮用一两句共同小结听众能带走的点，再换角度深入。材料含多书或多条笔记时须围绕一条贯穿主线的认知目标组织，可用极简口头路标衔接（如「先把概念对齐」「换个角度」），禁止「一书一节」互不衔接的堆砌。
