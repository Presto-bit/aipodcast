...
2. 每一行必须以「Speaker1: 」或「Speaker2: 」开头（数字 1 或 2，半角冒号，冒号后恰好一个空格）；禁止 Speaker3、Speaker4 或「主持人甲乙丙」等第三说话人标签。
...
【体裁】本文为书面长文，非播客节目稿；禁止「感谢收听」「感谢你的收听」「我们下次再见」「下期再见」「咱们下次再聊」等播客收束语；中途分段禁止告别、订阅引导或「本期节目」式套话。
【叙事与密度】材料含多来源时须围绕统一主线或核心问题递进、对照或归纳，段与段之间用因果、转折或递进衔接，避免资料块堆砌。每个主要论点先用一两句短句钉住结论或关键事实，再展开论述；避免全篇只有松散铺陈而无收束句。
【资料称谓】少用「《××》指出」「这本书认为」「某笔记写到」作主语反复点名；观点直接叙述，材料名尽量不在正文复述，溯源交给文末参考或角标。"""
...
严禁：播客式结语（如「感谢收听」「感谢你的收听」「我们下次再见」「咱们下次再聊」）；非全文最后一段时禁止任何告别语。
...
6. 若材料中有用户给出的结构要求（如分点、小结），请尽量满足。
   若材料含多个来源（多本书/多条笔记），须围绕统一主线或中心问题组织，将各来源观点对照、归纳或递进，避免写成彼此无关联的「资料块」堆砌；段与段之间须有因果、转折或递进式衔接，让读者感到在沿一条论证线推进而非并列粘贴。
{rules_tail}
{_zh_article_quality}
...
5. 文本要自然，包含适当的重复、语气词、停顿等真人对话特征；同时每个小话题至少用一两句短句钉住核心定义、数字或结论，再口语化展开，忌两人多轮只说同义废话、信息密度过低。
6. 【话轮与叙事】Speaker1 与 Speaker2 须功能互补（例如一方偏提问、过渡与微型小结，另一方偏拆解、举例与界定），避免轮流复述同一信息层；每隔若干轮用一两句共同小结听众能带走的点，再换角度深入。材料含多书或多条笔记时须围绕一条贯穿主线的认知目标组织，可用极简口头路标衔接（如「先把概念对齐」「换个角度」），禁止「一书一节」互不衔接的堆砌；台词里少用「《××》这本书」「某某资料写到」作主语反复点名，先把观点讲清楚，确需区分多方说法时可用「常见说法」「另一派观点」等中性表述，完整书名不必在口播里反复出现。
7. 对话正文中不要提及 AI 模型供应商、云平台或「MiniMax」等品牌字样，除非用户「材料内容」里原文已出现。
8. 【行格式必须严格遵守】全文只能输出对话行，不要标题、不要 Markdown、不要编号列表说明。
   - 每一行恰好对应一句对话；禁止在同一行内连写两轮不同说话人（禁止「Speaker2: …。Speaker1: …」粘在同一行，换说话人必须另起一行）。
   - 每行必须以英文标识开头：`Speaker1:` 或 `Speaker2:`（注意大小写与冒号），冒号后接该句台词；禁止 Speaker3/Speaker4 等额外说话人标签。
...
8) 总纲须体现两位说话人功能互补（如一方推进、过渡与小结节点，另一方拆解、举例与界定），避免各段只有平行罗列而无递进；若素材含多来源，各段 must_include 应服务同一条听众认知主线，勿设计成「一书一节」互不衔接。
...
【软着陆·文本】
- 开场：少用「大家好」「欢迎收听」「本期节目」连播式报幕；尽快给出「为什么现在值得听」的一点理由或小场景；末句略带承接感，像自然把话题交给后文，不要预告过长目录。
- 收场：先用一两句收束今天的主线或态度，再轻量致谢或下期预告；禁止在结尾突然展开全新大论点；句子宜短、少从句，最后几句略降信息密度。
