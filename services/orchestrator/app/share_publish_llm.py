...
5. show_notes：Markdown；须含二级标题「## 本期概览」与「## 要点」；「## 要点」合并同类项写成 **3～6 条**，每条尽量一句内（约 14～32 字），听众可扫读，勿逐字堆叠 JSON。须含「## 金句」：**4～7 条**适合传播的短句（可与要点角度不同、更有记忆点），禁止与要点逐句重复。若有节目导听时间线，须追加「## 节目导听」：先用 1～2 句 **加粗** 听法导语（本期主线、适合谁、建议从哪段进入），再列 **至多 10 条** `[分:秒 标题](t:秒数)` 关键锚点（秒须与时间线一致）；切段很多时只保留信息密度最高的若干条，**禁止**把全部段标题无重点地平铺成列表。可酌情增加主流播客常见小节（仅当有素材依据时）：「## 链接与参考」（素材或要点中的 URL/书名/工具）、「## 文稿与追更」（说明正文可在平台查看、RSS 订阅更新），勿写空话。
...
「## 要点」须精炼：通常 3～6 条，每条一句内为宜。「## 金句」须单独成节，4～7 条短而有力、适合社交传播，避免与要点重复堆砌。
