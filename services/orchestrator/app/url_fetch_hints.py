...
_DOMAIN_HINT_RULES: tuple[tuple[str, str], ...] = (
    (
        ...
        "你可：① 在浏览器中打开笔记，全选复制正文，使用下方「粘贴为笔记」；② 或「另存为」网页后上传 .html 文件。",
    ),
    (
        "xhslink.com",
        "短链跳转至小红书，同样无法在服务端拉取正文。\n"
        "请在小红书 App/网页内打开后复制正文，或使用「粘贴为笔记」/ 上传导出的 .html。",
    ),
    (
        "zhihu.com",
        "知乎长回答常需登录或反爬，链接导入可能失败。\n"
        "请复制回答正文到「粘贴为笔记」，或导出/打印为 PDF 后上传。",
    ),
    (
        "bilibili.com",
        "B 站视频页正文多为播放器与脚本，服务端无法替代字幕与转写。\n"
        "请复制简介/专栏文字到「粘贴为笔记」，或改用支持纯文本的专栏/文档链接。",
    ),
    (
        "weibo.com",
        "微博正文常需登录态，链接抓取不稳定。\n"
    ...
    (
        "twitter.com",
        "该站点限制匿名抓取。\n"
        "请复制推文内容到「粘贴为笔记」，或改用可公开访问的网页/文档链接。",
    ),
    (
        "x.com",
        "该站点限制匿名抓取。\n"
    ...
    (
        "linkedin.com",
        "LinkedIn 内容多在登录墙后。\n"
        "请复制正文到「粘贴为笔记」，或导出为 PDF 上传。",
    ),
    (
        "notion.so",
        "Notion 公开页若未发布为「公开到网络」或需登录，服务端可能拉取失败。\n"
        "请确认分享为公开链接，或复制内容到「粘贴为笔记」/ 导出 PDF。",
    ),
    (
        "feishu.cn",
        "飞书文档常需登录或内网权限。\n"
        "请使用「对外分享」可匿名访问的链接，或复制正文到「粘贴为笔记」。",
    ),
    (
        "larksuite.com",
        "飞书/ Lark 文档常需登录权限。\n"
        "请使用可匿名访问的分享链接，或复制正文到「粘贴为笔记」。",
    ),
)
...
def actionable_hint_for_failed_url(
    ...
    *,
    ...
    if error_code == "403" or "403" in err or "forbidden" in err:
        return (
            "站点拒绝了服务端访问（常见于反爬）。\n"
            "你可：① 在浏览器打开该页，复制正文后使用「粘贴为笔记」；② 或另存为 .html / 打印为 PDF 后本地上传。"
        )
    ...
    return (
        ...
        "建议：① 浏览器中打开页面，复制正文到「粘贴为笔记」；② 或上传 .html / .pdf 等本地文件。"
    )
