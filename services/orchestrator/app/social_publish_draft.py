...
结构硬性要求：
1. cover_hook：封面/列表钩子短语。
2. titles 数组（1 个主标题，≤20字）；也可用 title 单字段。
3. opening_30：正文开头句，总字数≤30（含标点）。
4. body：完整正文；段内用句号衔接，避免连续空行；可用 sections（每项 {{"heading":"小标题","content":"段落"}}）。
...
禁止：Speaker 对话格式、连续照抄 18 字以上、绝对化/医疗化/硬引流用语；禁止输出 directions/bodies/interactions 多方向数组。

只输出一个 JSON 对象，不要 markdown 代码块。键：
cover_hook, titles, title, opening_30, body, interaction, tags, imageSuggestions, theme"""
