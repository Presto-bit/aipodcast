...
_READABILITY_RULES = (
    "【可读性】\n"
    "1. 开篇单独一段：用 **加粗** 写 1～2 句直接结论（TL;DR），再空一行后展开。\n"
    "2. 正文用 ## 分 3～5 个小节（除非问题极短）；小节名贴合用户问题，勿用「可执行结论」「开篇结论」等模板标题。\n"
    "3. 每个 ## 下：先 1 句小结，再用 `-` 列表列 3～6 条要点；单条要点不超过 2 句。\n"
    "4. 单段正文不超过 6 句或约 120 字；禁止连续多段密排长散文。能列表则列表，勿大段复述原文。\n"
    "5. 按论证/主题组织，**不要**按来源或摘录顺序逐条复述「材料里有什么」。\n"
)

_SYSTEM_CORE = (
    "你是资料助手。仅依据用户提供的摘录作答；材料不足时明确说明「材料中未提及」或「摘录中看不到」，不要编造。\n"
    "使用中文与 GitHub 风格 Markdown。\n"
    f"{_READABILITY_RULES}"
...
_STYLE_HINTS: dict[str, str] = {
    "yesno": "【本题】开篇 **加粗** 是/否/视条件；正文 2～4 条 `-` 依据，禁止长段落。",
    "concept": "【本题】开篇 **加粗** 定义；正文：属性/边界用列表（3～5 条），必要时一小节区分易混项。",
    "howto": "【本题】开篇 **加粗** 结论或第一步；正文用有序列表写步骤（每步一行+一句目的）；末尾可列 1～3 条注意点。",
    "compare": "【本题】开篇 **加粗** 比较结论；正文必须用 Markdown 表格或「维度 + 列表」对比，禁止两侧长段落对照。",
    "survey": (
        "【本题】综述/介绍类：按问题维度分 3～4 个 ##（勿按资料目录顺序串讲）。"
        "每节 ≤1 句小结 + 3～6 条要点；每节正文（不含列表）合计宜 ≤150 字。禁止单节大段文字。"
    ),
    "general": "【本题】开篇 **加粗** 直接回答；并列/因果用 `-` 列表；需展开时用 2～4 个 ##，每节列表为主。",
}
...
_PLANNER_SYSTEM = (
    ...
    '"sections":[{"title":"小节标题","focus":"该节应回答什么（作答时用列表，3～6条）"}]}\n'
    "sections 2～4 项，每节 focus 宜短；多资料时 perSourceFocus 与来源序号 [n] 对齐；"
    "answerType 与问题形态一致；介绍/概述/全书结构类问题用 survey。"
)
...
def classify_answer_type(question: str) -> str:
    ...
    if re.search(r"(对比|区别|差异|vs\.?|VS|哪个好|选型|优缺点|孰优)", q, re.I):
        return "compare"
    if re.search(r"(全面|梳理|总结|综述|一览|架构|体系|通读|整体|介绍|概述|讲讲|讲了什么|主要内容|全书|结构)", q):
        return "survey"
...
def format_planner_block(plan: dict[str, Any]) -> str:
    ...
    lines = [
        "【写作计划】（据此组织正文，事实仍须来自下方摘录；遵守可读性：开篇加粗 TL;DR，各节用列表）",
        f"题型：{at}",
    ]
    if thesis:
